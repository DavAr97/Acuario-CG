...
glm::vec3 lightPos2(7.0f, 0.0f, 0.0f);
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.ambient"), 0.2f, 0.2f, 0.2f); //nos ilumina el escenario
		...
		lightColor.x= abs(sin(glfwGetTime() *Light1.x /4));
		lightColor.y= abs(sin(glfwGetTime() *Light1.y /4));
		lightColor.z= sin(glfwGetTime() *Light1.z /4);
		...
		lightColor2.x = abs(sin(glfwGetTime() * Light2.x / 4));
		lightColor2.y = abs(sin(glfwGetTime() * Light2.y / 4));
		lightColor2.z = sin(glfwGetTime() * Light2.z / 4);
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_I]) {
		spotLightDir.x += 0.04f;
	}
	if (keys[GLFW_KEY_K]) {
		spotLightDir.x -= 0.04f;
	}
	if (keys[GLFW_KEY_O]) {
		spotLightDir.y += 0.04f;
	}
	if (keys[GLFW_KEY_L]) {
		spotLightDir.y -= 0.04f;
	}
	if (keys[GLFW_KEY_Z]) {
		spotLightDir.z += 0.04f;
	}
	if (keys[GLFW_KEY_X]) {
		spotLightDir.z -= 0.04f;
	}
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_SPACE])
	{
		...
		if (active)
		{
			Light1 = glm::vec3(1.0f, 0.64f, 0.0f);
